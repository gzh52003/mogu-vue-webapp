  ...
  >
    <template #content>
      <van-grid v-if="active === 0" :border="false" :column-num="1">
        <van-grid-item v-for="item in recommendone" :key="item.iid" @click="gotoDea(item.iid)">
      ...
      <van-grid v-if="active === 1" :border="false" :column-num="2">
        <van-grid-item v-for="item in recommendtwo" :key="item.iid" @click="gotoDea(item.iid)">
